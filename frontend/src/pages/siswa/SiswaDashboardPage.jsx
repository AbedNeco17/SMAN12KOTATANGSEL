          ...
          <div className="text-center md:text-left min-w-0 flex-1 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-xs sm:text-sm text-slate-500 leading-relaxed max-w-2xl">
                Akumulasi poin pelanggaran Anda saat ini adalah <strong className="text-slate-800">{studentPoints.poin_pelanggaran || 0} dari maksimal 100 poin</strong>. Status kesiswaan Anda saat ini adalah <span className={`px-2 py-0.5 rounded text-xs font-bold font-mono ${
                  studentPoints.poin_pelanggaran > 0 ? 'bg-red-500/10 text-red-600' : 'bg-emerald-500/10 text-emerald-600'
                }`}>{studentPoints.status || 'Bebas Pelanggaran'}</span>. Hindari pelanggaran dan kumpulkan penghargaan!
              </p>
            </div>
            <button
              ...
              <div className="space-y-3 text-xs">
                <div>
                  <div className="flex justify-between text-slate-600 mb-1">
                    <span>Batas Poin Maksimal</span>
                    <span className="font-bold text-slate-800">{studentPoints.poin_pelanggaran || 0} / 100</span>
                  </div>
                  <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                    <div className="bg-red-500 h-full rounded-full" style={{ width: `${Math.min(100, Math.max(0, studentPoints.poin_pelanggaran || 0))}%` }} />
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1.5">
                    {(studentPoints.poin_pelanggaran || 0) >= 100 ? (
                      <span className="text-red-600 font-semibold">Batas maksimal poin telah tercapai. Harap segera melapor ke BK.</span>
                    ) : (
                      <>
                        Batas toleransi tersisa <strong className="text-slate-600">{100 - (studentPoints.poin_pelanggaran || 0)} poin</strong> pelanggaran sebelum tindakan panggilan terakhir.
                      </>
                    )}
                  </p>
                </div>
